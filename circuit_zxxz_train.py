...
class SurfaceCode:
  ...
  def __init__(self, seed, git_version=0, distance=3,
              ...
              pix=0, piy=0, piz=0, pxi=0, pyi=0, pzi=0, pxx=0, pxy=0,
              pxz=0, pyx=0, pyy=0, pyz=0, pzx=0, pzy=0, pzz=0):
    ...
    self.pm = pm
    self.pix, self.piy, self.piz, self.pxi, self.pyi, self.pzi = pix, piy, piz, pxi, pyi, pzi
    self.pxx, self.pxy, self.pxz, self.pyx, self.pyy, self.pyz = pxx, pxy, pxz, pyx, pyy, pyz
    self.pzx, self.pzy, self.pzz = pzx, pzy, pzz
  ...
  def _do_step_1(self):
    ...
    CNOT gates are applied to the z-ancillas. Both data and ancilla qubits 
    experience two-qubit gate errors.
    """
    self._do_cnot_north_step(self.x_north_dict, self.z_north_dict)
    self._appy_two_qbits_errors(self.data_l, self.x_anc_l+self.z_anc_l)

  def _do_step_2(self):
    """ This function executes the second step of the circuit model. During
    this step the x-ancillas and z-ancillas undergo a Hadamard rotation. 
    Both, data- and ancilla qubits experience uncorrelated errors.
    """

    # Apply a Hadamard gate on the x- and z-ancillas
    self._hadamard_on_x_ancs()
    self._hadamard_on_z_ancs()
    
  def _do_step_3(self):
    """ This function executes the third step of the circuit model. During
    this step west CNOT gates are applied to the x-ancillas, and east
    CNOT gates are applied to the z-ancillas. Both data and ancilla qubits 
    experience two-qubit gate errors.
    """
    self._do_cnot_west_east_step(self.x_west_dict, self.z_east_dict)
    self._appy_two_qbits_errors(self.data_l, self.x_anc_l+self.z_anc_l)
    
    
  def _do_step_4(self):
    """ This function executes the third step of the circuit model. During
    this step east CNOT gates are applied to the x-ancillas, and west
    CNOT gates are applied to the z-ancillas. Both data and ancilla qubits 
    experience two-qubit gate errors.
    """
    self._do_cnot_east_west_step(self.x_east_dict, self.z_west_dict)
    self._appy_two_qbits_errors(self.data_l, self.x_anc_l+self.z_anc_l)
  ...
  def _do_step_6(self):
    """ This function executes the sith step of the circuit model. During
    this step south CNOT gates are applied to the x-ancillas, and south
    CNOT gates are applied to the z-ancillas. Both data and ancilla qubits 
    experience two-qubit gate errors.
    """
    self._do_cnot_south_step(self.x_south_dict, self.z_south_dict)
    self._appy_two_qbits_errors(self.data_l, self.x_anc_l+self.z_anc_l)

  def _do_measure_step(self):
    """ This function executes the seventh step. During this step the ancilla
    qubits are measured (with measurement errors). The measurement
  ...
  def _do_cnot_north_step(self, x_dict, z_dict):
    """ This function executes one of the CNOT steps. It applies the CNOT
    operations for both ancilla and data qubits for north data qubits.
    Input
    -----
    ...
    # Apply CNOTS
    self._do_north_cnots(x_dict, 'x')
    self._do_north_cnots(z_dict, 'z')


    
  def _do_cnot_south_step(self, x_dict, z_dict):
    """ This function executes one of the CNOT steps. It applies the CNOT
    operations for both ancilla and data qubits for south data qubits.
    Input
    -----
    x-dict -- a dictionary containing the connections set by the CNOT
              gates between x-ancillas and data qubits
    ...
    # Apply CNOTS
    self._do_south_cnots(x_dict, 'x')
    self._do_south_cnots(z_dict, 'z')
    
    
    
  def _do_cnot_east_west_step(self, x_dict, z_dict):
    """ This function executes one of the CNOT steps. It applies the CNOT
    operations for both ancilla and data qubits for the east and west gates.
    Input
    -----
    x-dict -- a dictionary containing the connections set by the CNOT
              gates between x-ancillas and data qubits
    ...
    # Apply CNOTS
    self._do_east_cnots(x_dict, 'x')
    self._do_west_cnots(z_dict, 'z')

    
  def _do_cnot_west_east_step(self, x_dict, z_dict):
    """ This function executes one of the CNOT steps. It applies the CNOT
    operations for both ancilla and data qubits for west and east gates.
    Input
    -----
    x-dict -- a dictionary containing the connections set by the CNOT
              gates between x-ancillas and data qubits
    ...
    # Apply CNOTS
    self._do_west_cnots(x_dict, 'x')
    self._do_east_cnots(z_dict, 'z')
  ...
  def _appy_two_qbits_errors(self, dat_qubits, anci_qubits):
    """ This function applies two qubits gate errors to the qubits.

    Input
    -----
    dat_qubits -- a list with all the data qubits that are subject to two qubits
              errors 
              
    anci_qubits -- a list with all the ancilla qubits that are subject to two qubits
              errors
    """
    for dqb in dat_qubits:
      for aqb in anci_qubits:
        # we throw the dice fifteen times for the 15 two qubits errors
        rn_ix, rn_iz, rn_iy = self.rng.rand(), self.rng.rand(), self.rng.rand()
        rn_xi, rn_zi, rn_yi = self.rng.rand(), self.rng.rand(), self.rng.rand()
        rn_xx, rn_xy, rn_xz = self.rng.rand(), self.rng.rand(), self.rng.rand()
        rn_yx, rn_yy, rn_yz = self.rng.rand(), self.rng.rand(), self.rng.rand()
        rn_zx, rn_zy, rn_zz = self.rng.rand(), self.rng.rand(), self.rng.rand()
            
        if rn_ix < self.pix:
            self.anc_qubits[aqb][0] = not self.anc_qubits[aqb][0]
                    
        elif rn_iz < self.piz:
            self.anc_qubits[aqb][1] = not self.anc_qubits[aqb][1]
            ...
            self.data_qubits[dqb][0] = not self.data_qubits[dqb][0]
                    
        elif rn_zi < self.pzi:
            self.data_qubits[dqb][1] = not self.data_qubits[dqb][1]
  ...
  def _hadamard_on_z_ancs(self):  # test written
    """ This function applies a Hadamard gate to the x-ancillas. This gate
    exchanges bitflip- and phaseflip-errors, i.e. x <--> z errors.
    Y errors get a global phase that we can ignore.
    """
  ...
  def _do_north_cnots(self, cnots, which_anc):
    """ This function executes the north CNOT gate.
    For x-ancillas: ancilla qubits are target bits, data qubits are
                control bits
  ...
  def _do_south_cnots(self, cnots, which_anc):
    """ This function executes the south CNOT gate. 
    For x-ancillas: ancilla qubits are target bits, data qubits are
                control bits
    For z-ancillas: ancilla qubits are target bits, data qubits are
                control bits
  ...
  def _do_west_cnots(self, cnots, which_anc):
    """ This function executes the west CNOT gate.
    For x-ancillas: ancilla qubits are control bits, data qubits are
                    target bits
    For z-ancillas:  ancilla qubits are control bits, data qubits are
                    target bits
    Input
    -----
    cnots -- a dictionary that describes how the cnots are connecting
  ...
  def _do_east_cnots(self, cnots, which_anc):
    """ This function executes the east CNOT gate.
    For x-ancillas: ancilla qubits are control bits, data qubits are
                    target bits
    For z-ancillas:  ancilla qubits are control bits, data qubits are
                    target bits
...
if __name__ == '__main__':
  ...
  db_path ="C:\\Users\\Lucrece\\Documents\\docs_phd\\decoder\\"
  ...
  ## there are 15 two-qbit errors. The error probability of two_qubit
  # gate on data qubits
  pt = p_per_step/15
  ...
  # two qubits errors. the error probability per step
  pix, piy, piz = pt, pt, pt
  pxi, pyi, pzi = pt, pt, pt
  pxx, pxy, pxz = pt, pt, pt
  pyx, pyy, pyz = pt, pt, pt
  pzx, pzy, pzz = pt, pt, pt
  ...
  print("Two qubit error probability on the qubits in percent: (pix, piy, piz) =",
        round(pix * 100, 4), round(piy * 100, 4), round(piz * 100, 4))
  print("Two qubit error probabilty on the qubits in percent: (pxi, pyi, pzi) = ",
        round(pxi * 100, 4), round(pyi * 100, 4), round(pzi * 100, 4))
  print("Two qubit error probabilty on the qubits in percent: (pxx, pxy, pxz) = ",
        round(pxx * 100, 4), round(pxy * 100, 4), round(pxz * 100, 4))
  print("Two qubit error probabilty on the qubits in percent: (pyx, pyy, pyz) = ",
        round(pyx * 100, 4), round(pyy * 100, 4), round(pyz * 100, 4))
  print("Two qubit error probabilty on the qubits in percent: (pzx, pzy, pzz) = ",
        round(pzx * 100, 4), round(pzy * 100, 4), round(pzz * 100, 4))
